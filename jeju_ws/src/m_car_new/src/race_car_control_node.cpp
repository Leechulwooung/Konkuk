...
#define STEER_NEUTRAL_ANGLE 120

#define RAD2DEG(x) ((x)*180./M_PI)
#define DEG2RAD(x) ((x)/180.*M_PI)
// unit : m
//#define Sonar_Detect_Range 0.3  // 30

////////////change sonar///////////////
#define Sonar_Detect_Range 230
///////////////////////////////////////

#define LIDAR_Obastacle   0.6
#define LIDAR_Wall_Detection 0.30

int  Base_Speed = 130; // ********************************************************만져야 하는 속도 변화
//int  Base_Speed = 0;
...
//  *****************teleop,py 에서 받아오는 steer값, speed 값 
...
int main(int argc, char **argv)
{
  ...
  //ros::Subscriber sub2 = n.subscribe("/Car_Control_cmd/V_SteerAngle_Int16",10, &CarSteerControlCallback);  
  ros::Subscriber sub2 = n.subscribe("/Car_Control_cmd/SteerAngle_Int16",10, &CarSteerControlCallback);  
 ...
 // ros::Publisher car_control_pub4 = n.advertise<std_msgs::Int16>("Car_Control/Speed_Int16", 10);
  ...
  ////////change sonar//////////////
  ros::Publisher car_control_pub55 = n.advertise<sensor_msgs::Range>("/RangeSonar1",10);
  //////////////////////////////////
  ...
  */
  ...
  sonar_msg1.header.frame_id =  frameid;
  sonar_msg1.radiation_type = sensor_msgs::Range::ULTRASOUND;
  sonar_msg1.field_of_view = (30.0/180.0) * 3.14;
  sonar_msg1.min_range = 0.0;
  sonar_msg1.max_range = 1.50;  //[unit :m]
  
  sensor_msgs::Range sonar_msg2;
  sonar_msg2.header.frame_id =  frameid;
  sonar_msg2.radiation_type = sensor_msgs::Range::ULTRASOUND;
  sonar_msg2.field_of_view = (30.0/180.0) * 3.14;
  sonar_msg2.min_range = 0.0;
  sonar_msg2.max_range = 1.50;  //[unit :m]
  ///////////////////////////////////////////
  ...
  while (ros::ok())
  {
   ...
   // car_control_pub2.publish(msg);
    ...
    else
    {
		//ROS_INFO("Tlqkf");
		motor_speed = Base_Speed;
		//motor_speed = 110;		// ***************************************우리가 제어하는 soeed 값을 받아오는 변수로 바꿈
	} 
   ...
   */
   ...
   //////////////////change sonar///////////////////
    sonar_msg1.header.stamp = ros::Time::now();    
    sonar_msg1.range = sonar[0];
    car_control_pub5.publish(sonar_msg1);
    
    sonar_msg2.header.stamp = ros::Time::now();    
    sonar_msg2.range = sonar[1];
    car_control_pub55.publish(sonar_msg2);
