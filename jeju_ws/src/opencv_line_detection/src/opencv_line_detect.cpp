//opencv_line_detect.cpp
//-I/usr/local/include/opencv4/opencv -I/usr/local/include/opencv4

#include <ros/ros.h>
#include "std_msgs/String.h"
#include "std_msgs/Int16.h"


////////////////////////////
//#include "image_transport/image_transport.h"
//#include "cv_bridge/cv_bridge.h"
//////////////////////////////////////

#include <opencv2/opencv.hpp>
#include <iostream>
#include <stdio.h> 
#include <string.h> 
#include <stdio.h>
#include <unistd.h>   
#include <stdint.h>   
#include <stdlib.h>  
#include <errno.h>


/////////////////////////////
#include <limits>
#include <math.h>
/////////////////////////////

using namespace cv;
using namespace std;

#define IMG_Width     1280
#define IMG_Height    720

#define USE_DEBUG  1   // 1 Debug  사용
#define USE_CAMERA 1   // 1 CAMERA 사용  0 CAMERA 미사용
#define ROI_CENTER_Y  300 //****************************************************
#define ROI_WIDTH     60  //****************************************************

#define NO_LINE 20

std::string gstreamer_pipeline (int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method) {
    return "nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)" + std::to_string(capture_width) + ", height=(int)" +
           std::to_string(capture_height) + ", format=(string)NV12, framerate=(fraction)" + std::to_string(framerate) +
           "/1 ! nvvidconv flip-method=" + std::to_string(flip_method) + " ! video/x-raw, width=(int)" + std::to_string(display_width) + ", height=(int)" +
           std::to_string(display_height) + ", format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink";
}


Mat mat_image_org_color;  // Image 저장하기 위한 변수
//Mat mat_image_org_gray;
////////////////////////////
Mat mat_image_org_gray;
Mat mat_image_org_hsv;
Mat binary;
Mat mask;
Mat mat_image_org_morp;
///////////////////////////
Mat mat_image_roi;
Mat mat_image_canny_edge;
Mat mat_image_canny_edge_roi;

Scalar GREEN(0,255,0);
Scalar RED(0,0,255);
Scalar BLUE(255,0,0);
Scalar YELLOW(0,255,255);


int img_width = 640;
int img_height = 480;

Mat Region_of_Interest(Mat image, Point *points)
{
  Mat img_mask =Mat::zeros(image.rows,image.cols,CV_8UC1);	 
  
  Scalar mask_color = Scalar(255,255,255);
  const Point* pt[1]={ points };	    
  int npt[] = { 4 };
  cv::fillPoly(img_mask,pt,npt,1,Scalar(255,255,255),LINE_8);
  Mat masked_img;	
  bitwise_and(image,img_mask,masked_img);
  
  return masked_img;
}

Mat Region_of_Interest_crop(Mat image, Point *points)
{
   Mat img_roi_crop;	

   Rect bounds(0,0,image.cols,image.rows);	 
   Rect r(points[0].x,points[0].y,image.cols, points[2].y-points[0].y);  
   //printf("%d %d %d %d\n",points[0].x,points[0].y,points[2].x, points[2].y-points[0].y);
   //printf("%d  %d\n", image.cols, points[2].y-points[0].y);

   img_roi_crop = image(r & bounds);
   
   return img_roi_crop;
}

Mat Canny_Edge_Detection(Mat img)
{
   Mat mat_blur_img, mat_canny_img;
   //blur(img, mat_blur_img, Size(3,3));	
   //GaussianBlur(img, mat_blur_img, Size(5,5),0,0);
   medianBlur(img, mat_blur_img, 9);
   Canny(mat_blur_img,mat_canny_img, 70,170,3);
	
   return mat_canny_img;	
}


int main(int argc, char **argv)
{
   int i;
   int steer_angle_new, steer_angle_old;
   int steer_angle_real;
   
   steer_angle_new = steer_angle_old =0; 
 
   float gradient[NO_LINE]  = {0,};
   float intersect[NO_LINE] = {0,};
   float intersect_base[NO_LINE]  = {0,};
   float c_x_sum=0;	
   
   int capture_width = 640; //1280 ;
   int capture_height = 360; //720 ;
   int display_width = 640 ;
   int display_height = 360 ;
   int framerate = 60 ;
   int flip_method = 2 ;
    
   int img_width  = 640;
   int img_height = 360;
   if(USE_CAMERA == 0) img_height = 480;	 
   
   std::cout << "OpenCV version : " << CV_VERSION << std::endl;
  // return 1;
    
   std::string pipeline = gstreamer_pipeline(capture_width,
	capture_height,
	display_width,
	display_height,
	framerate,
	flip_method);
    std::cout << "Using pipeline: \n\t" << pipeline << "\n";
 
   cv::VideoCapture cap(pipeline, cv::CAP_GSTREAMER);
  
   
  
   Point points[4];  // ROI(Region of Interest) 
    //cap = VideoCapture(0, cv::CAP_V4L2);
   
   if (!cap.isOpened()) 
   {
	cerr << "에러 - 카메라를 열 수 없습니다.\n";		
	return -1;	
   }
   
   cap.read(mat_image_org_color);
   img_width = mat_image_org_color.size().width ;
   img_height = mat_image_org_color.size().height;
	
   if(USE_CAMERA == 0) printf("Image size[%3d,%3d]\n", capture_width,capture_height);	
  
  
   ros::init(argc, argv, "ros_opencv_line");

  /**
   * NodeHandle is the main access point to communications with the ROS system.
   * The first NodeHandle constructed will fully initialize this node, and the last
   * NodeHandle destructed will close down the node.
   */
   ros::NodeHandle nh;
   
   ros::Publisher car_control_pub_cmd = nh.advertise<std_msgs::Int16>("Car_Control_cmd/SteerAngle_Int16", 10);
   
   std_msgs::Int16 cmd_steering_msg;      
   cmd_steering_msg.data  = 0;
   
   std::cout << "OpenCV version : " << CV_VERSION << std::endl;
  
   ros::Rate loop_rate(10);
   int count = 0;
   
   float  c[NO_LINE] = {0.0, };
   float  d[NO_LINE] = {0.0, };
   float  line_center_x = 0.0; 
   float  steering_conversion = 0.343; //***************************값 바꿈
   float  a = 1.0;
   
   /*
   image_transport::ImageTransport it(nh);
   image_transport::Publisher pub = it.advertise("camera/image", 1);
   */
   
   namedWindow("Camera Image", WINDOW_NORMAL);            //NOGRAPHIC
   resizeWindow("Camera Image", IMG_Width/2,IMG_Height/2);  //NOGRAPHIC
   moveWindow("Camera Image", 10, 10);                               //NOGRAPHIC
                     
   namedWindow("Gray Image window", WINDOW_NORMAL);   
   resizeWindow("Gray Image window", img_width,img_height);   
   moveWindow("Gray Image window", 700, 10);
   
    namedWindow("HSV Image window", WINDOW_NORMAL);   
   resizeWindow("HSV Image window", img_width,img_height);   
   moveWindow("HSV Image window", 700, 10);
   
   namedWindow("MASK Image window", WINDOW_NORMAL);   
   resizeWindow("MASK Image window", img_width,img_height);   
   moveWindow("MASK Image window", 700, 10);
   /*
   namedWindow("ROI Image window", WINDOW_NORMAL);   
   resizeWindow("ROI Image window", img_width,img_height);   
   moveWindow("ROI Image window", 10, 500);
    */
   namedWindow("Edge Image window", WINDOW_NORMAL);   
   resizeWindow("Edge Image window", img_width,img_height);   
   moveWindow("Edge Image window", 700, 500);
    
   points[0] = Point(0,ROI_CENTER_Y-ROI_WIDTH);
   points[1] =  Point(0,ROI_CENTER_Y+ROI_WIDTH);
   points[2] =  Point(img_width,ROI_CENTER_Y+ROI_WIDTH);
   points[3] =  Point(img_width,ROI_CENTER_Y-ROI_WIDTH);
   
   //sensor_msgs::ImagePtr img_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", mat_image_org_color).toImageMsg();
   
    
   while (ros::ok())
   { 
    /**
     * This is a message object. You stuff it with data, and then publish it.
     */
     if(USE_CAMERA == 1)  cap.read(mat_image_org_color);
     else                 mat_image_org_color = imread("./img/line_1.jpg", IMREAD_COLOR);
     
     /*
     cvtColor(mat_image_org_color, mat_image_org_gray, cv::COLOR_RGB2GRAY);        // color to gray conversion  
     mat_image_roi = Region_of_Interest_crop(mat_image_org_gray,points);    // ROI ?ì­??ì¶ì¶??     
     mat_image_canny_edge = Canny_Edge_Detection(mat_image_roi);
      */
      //////////////////Grayscale
      /*
      Mat element = getStructuringElement(MORPH_RECT, Size(5,5));
      cvtColor(mat_image_org_color, mat_image_org_gray, COLOR_BGR2GRAY);
      equalizeHist(mat_image_org_gray, mat_image_org_gray);
      threshold(mat_image_org_gray, mask, 240, 248, THRESH_BINARY);
      morphologyEx(mask, mat_image_org_morp, MORPH_CLOSE, element, Point(-1,-1),12);
      */
      
      ////////////////HSVscale
      
      cvtColor(mat_image_org_color, mat_image_org_gray, COLOR_BGR2GRAY);
      equalizeHist(mat_image_org_gray, mat_image_org_gray);
      adaptiveThreshold(mat_image_org_gray, binary, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 11, 2);
     
      cvtColor(mat_image_org_color, mat_image_org_hsv, COLOR_BGR2HSV);
     inRange(mat_image_org_hsv, Scalar(20, 80,80), Scalar(40, 220,255), mask);
     morphologyEx(mask,mask,MORPH_CLOSE, Mat(), Point(-1,-1), 2);
      Mat combined_mask = mask & binary;
     // inRange(mat_image_org_gray, Scalar(250,250,250), Scalar(255,255,255), mask);
     
     
     /////////////yellow_weight
     /*
     Mat element = getStructuringElement(MORPH_RECT, Size(5, 5));
      
      cvtColor(mat_image_org_color, mat_image_org_gray, COLOR_BGR2GRAY);
      Mat weighted_gray = Mat::zeros(mat_image_org_gray.size(), mat_image_org_gray.type());
      
      for (int i = 0; i < mat_image_org_color.rows; i++) {
          for (int j = 0; j < mat_image_org_color.cols; j++) {
              double r_weight = 0.5;
              double g_weight = 0.5;
              double b_weight = 1.0;
              weighted_gray.at<uchar>(i, j) = static_cast<uchar>((r_weight * mat_image_org_color.at<Vec3b>(i, j)[2] + g_weight * mat_image_org_color.at<Vec3b>(i, j)[1] + b_weight * mat_image_org_color.at<Vec3b>(i, j)[0]) / (r_weight + g_weight + b_weight));
          }
      }

      int yellow_threshold = 180;
      Mat binary_output = Mat::zeros(weighted_gray.size(), CV_8UC1);
      threshold(weighted_gray, binary_output, yellow_threshold, 255, THRESH_BINARY);
      morphologyEx(binary_output, mat_image_org_morp, MORPH_CLOSE, element, Point(-1, -1), 15);
     */


      //Mat mat_image_org_yellow = cv::Mat::zeros(mat_image_org_color.rows, mat_image_org_color.cols, CV_8UC1);
      
      //cvtColor(mat_image_org_color, mat_image_org_gray, COLOR_BGR2GRAY);
      
      //ouble weights[] = {0.4,0.6,0.1};
      //Mat mat_image_org_yellow = mat_image_org_color * Mat(3,1, CV_64F, weights};
      
      
      //mat_image_roi = Region_of_Interest_crop(combined_mask, points);
       mat_image_roi = Region_of_Interest_crop(combined_mask, points);
       mat_image_canny_edge = Canny_Edge_Detection(mat_image_roi);
      
      
     vector<Vec4i> linesP;
     HoughLinesP(mat_image_canny_edge, linesP, 1, CV_PI/180,30,30,40);
     printf("Line Number : %3d\n", (int)linesP.size());
     
     line_center_x = 0.0;
	  
	  for(int i=0; i<linesP.size();i++)
	  {
		float intersect = 0.0;
		
		if(i>=NO_LINE) break;
		Vec4i L= linesP[i];
		/*
		int cx1 = linesP[i][0];
		int cy1 = linesP[i][1];
		int cx2 = linesP[i][2];
		int cy2 = linesP[i][3];
		*/
		c[i] =  (L[2]-L[0])/(L[3]-L[1]);
		d[i] = L[0]-c[i] *L[1];
		
		
		//////////////내가 만든 코드/////////////
		/*
		if (fabs(c[i]) < fabs(2)){
			
			printf("라인이 검출 됨\n");
			
			d[i] = L[0]-c[i] *L[1];
			
			intersect = c[i] * (float)ROI_WIDTH + d[i];

			line_center_x += intersect;
			line(mat_image_org_color, Point(L[0], L[1] + ROI_CENTER_Y - ROI_WIDTH), Point(L[2], L[3] + ROI_CENTER_Y - ROI_WIDTH), Scalar(0, 0, 255), 3, LINE_AA);
			
		}
		*/
		/*//////////////////////////////
		else {
			printf("라인이 검출 안 됨\n");
			//c[i] = ;
			d[i] = 0;
			intersect = 0;
			line_center_x += intersect;
		}
		*///////////////////////////////
		/*
		else if(fabs(c[i]) == std::numeric_limits<double>::infinity()){
				printf("라인이 검출 안 됨2222\n");
				//c[i] = ;
				d[i] = 0;
				intersect = 0;
				line_center_x += intersect;
		}
		else {
			printf("라인이 검출 안 됨\n");
			//c[i] = ;
			d[i] = 0;
			intersect = 0;
			line_center_x += intersect;
		}
		*/

		////////////////////////////////////
		
		
		intersect = c[i]*(float)ROI_WIDTH +d[i];
		line_center_x += intersect;
		
		line(mat_image_org_color,Point(L[0],L[1]+ROI_CENTER_Y-ROI_WIDTH),Point(L[2],L[3]+ROI_CENTER_Y-ROI_WIDTH), Scalar(0,0,255), 3, LINE_AA);		   
		
		
		printf("L[%d] :[%3d, %3d] , [%3d , %3d] \n",i,  L[0],L[1], L[2],L[3]); 
		//printf("x[%d] = [%6.3f] *y + [%6.3f] \n",i, c[i],d[i]);
		printf("x[%d] = [%f] *y + [%f] \n", i,c[i],d[i]);
		printf("intersect =[%f], line_center_x_sum : [%f]\n\n", intersect, line_center_x);
		//printf("H :[%3d , %3d] , [%3d , %3d] \n", cx1,cy1, cx2,cy2);
	   } 
	   
	  
      
     if(linesP.size() != 0) 
     {
        
        line_center_x = line_center_x / (float)linesP.size() - img_width/2;

	    //steering_conversion = (fabs(line_center_x)*0.009 + 0.061);
	
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	/*
	    if (line_center_x < -50 || line_center_x > 50) 
        {
			steering_conversion = 0.43;
		}
		else {
			steering_conversion = 0.07;
		}
		*/
       printf("*****st-conversion : [%f] \n", steering_conversion);
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	
        steer_angle_new = (int)( line_center_x*steering_conversion);  //스티어링 조정값 계수 수정 필요 
        steer_angle_real = a*(steer_angle_new) + (1-a)*(steer_angle_old);
        printf("c_x_sum = %f  %d\n",line_center_x , steer_angle_new);
	    printf("\n\n\n");
        cmd_steering_msg.data  = steer_angle_real;    //
        
        /*
        int dif = steer_angle_new - steer_angle_old;
        if(steer_angle_old !=  steer_angle_new) {//이전 값이랑 다를 때
			if(fabs(dif)<=80){
				car_control_pub_cmd.publish(cmd_steering_msg);
			}// 이전 값이랑 다른데, 차이가 작을 
			else {
				ROS_INFO("차이 ");
			} //차이가 클 때(직노이즈 검출)
		}
		*/
		if(steer_angle_old !=  steer_angle_real) car_control_pub_cmd.publish(cmd_steering_msg);
	
	    line(mat_image_org_color,Point(0,ROI_CENTER_Y),Point(img_width,ROI_CENTER_Y), Scalar(0,255,0), 1, LINE_AA);	
	    line(mat_image_org_color,Point((int)line_center_x+img_width/2,ROI_CENTER_Y-ROI_WIDTH),Point((int)line_center_x+img_width/2,ROI_CENTER_Y+ROI_WIDTH), Scalar(255,255,0), 1, LINE_AA);	
	   
     }
      
     else
     {
         ROS_INFO("Tlqkfgggggg");
         steer_angle_new = steer_angle_old;
         cmd_steering_msg.data = steer_angle_new;
         car_control_pub_cmd.publish(cmd_steering_msg);
         
         
     }
       steer_angle_old =  steer_angle_new ;   
       
       /*
       img_msg  = cv_bridge::CvImage(std_msgs::Header(), "bgr8", mat_image_org_color). toImageMsg();
       pub.publish(img_msg);
       */
     
      imshow("Camera Image", mat_image_org_color);               //NOGRAPHIC
      imshow("Gray Image window", mat_image_org_gray); 
      //imshow("HSV Image window", mat_image_org_hsv); 
      imshow("MASK Image window", mask); 
    //imshow("ROI Image window",mat_image_roi);
      imshow("Edge Image window",mat_image_canny_edge);  
    
    if (waitKey(25) >= 0)
      break;
     
     ros::spinOnce();

    loop_rate.sleep();
    ++count;
  }

  destroyWindow("Camera Image");                                        //NOGRAPHIC
  destroyWindow("Gray Image window");

  //destroyWindow("HSV Image window");
  //destroyWindow("ROI Image window");
  destroyWindow("Edge Image window");
   return 0;
}

 
